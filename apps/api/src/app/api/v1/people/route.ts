...
const updatePersonSchema = z.object({
  ...
  allowBirthdaySms: z.boolean().optional(),
  allowEventSms: z.boolean().optional(),
  allowWelfareSms: z.boolean().optional(),
  allowWhatsApp: z.boolean().optional(),
  allowBirthdayWhatsApp: z.boolean().optional(),
  ...
  try {
    ...
    const updated = await prisma.person.update({
      ...
      data: {
        ...
        whatsappNumber: input.whatsappNumber === undefined ? existing.whatsappNumber : input.whatsappNumber || null,
        allowSms: input.allowSms ?? existing.allowSms,
        allowBirthdaySms: input.allowBirthdaySms ?? existing.allowBirthdaySms,
        allowEventSms: input.allowEventSms ?? existing.allowEventSms,
        allowWelfareSms: input.allowWelfareSms ?? existing.allowWelfareSms,
        allowWhatsApp: input.allowWhatsApp ?? existing.allowWhatsApp,
        allowBirthdayWhatsApp: input.allowBirthdayWhatsApp ?? existing.allowBirthdayWhatsApp,
        preferredCommunicationChannel: input.preferredCommunicationChannel ?? existing.preferredCommunicationChannel,
        doNotContact: input.doNotContact ?? existing.doNotContact,
      },
      include: {
        member: true,
        familyMembers: { include: { family: true }, take: 1 },
