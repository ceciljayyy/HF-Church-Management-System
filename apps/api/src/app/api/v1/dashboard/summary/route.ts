  ...
  try {
    ...
    const summary = await getOrSetCache(
      ...
      async () => {
        ...
        const [peopleHistory, attendanceHistory, upcomingEvents, recentActivities, groupStats] = await Promise.all([
        ...
        const peopleGrowthSeries = months.map((month) => {
          ...
          return {
            ...
            value: peopleHistory.filter((item) => monthKey(item.createdAt) === key).length,
          };
        });
        ...
        const attendanceSeries = months.map((month) => {
          ...
          return {
            name: monthLabel(month),
            value: attendanceHistory.filter((item) => monthKey(item.checkedInAt) === key).length,
          };
        });
